...
using Craftsmaneer.DataToolUtils.Properties;
...
namespace Craftsmaneer.DataToolUtils
{
    public partial class DataToolUtilForm : Form
    {
        ...
        {
            IEnumerable<string> tableNames = lvwTables.CheckedItems.Cast<ListViewItem>().Select(lvi => lvi.Text);
            var doc = new XDocument();
            doc.Add(new XElement("DataTableSet",
                new XComment(string.Format("Generated by DataFormUtil @ {0}.", DateTime.Now))));
            ...
            doc.Save(string.Format(@"{0}\DataTableSet.config", txtExportPath.Text));
        }
        ...
        {
            IEnumerable<DataTable> datatables = lvwTables.GetDataSet().Tables.Cast<DataTable>();
            foreach (DataTable table in datatables)
            {
                string path = string.Format(@"{0}\{1}.xml", txtExportPath.Text, table.TableName);
        ...
        private void cmdImportDataTable_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                string tableName = fi.Name.Substring(0, (fi.Name.Length - fi.Extension.Length));
        ...
        private void HighlightDifferences(DataGridView dgvImported, TableDiff delta)
        {
            ...
            IEnumerable<DataGridViewRow> rows = dgvImported.Rows.Cast<DataGridViewRow>();

            foreach (RowDiff item in delta.RowDiffs)
            {
